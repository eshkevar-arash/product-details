...
function showProducts(){
    ...
    products.forEach(product => {
        ...
        newDiv.innerHTML = `
            <img src="${product.img}" alt="لپ تاپ لنوو">
            <div class="product-info">
                <h3>${product.title}</h3>
                <p>${product.description}</p>
                <div class="product-price">${product.price} تومان</div>
            </div>
            <button class="detail-btn" data-id = ${product.id}>نمایش جزئیات</button>
        `
        fragmentElem.appendChild(newDiv)
    })
    productsWrapper.appendChild(fragmentElem)
}
showProducts()
